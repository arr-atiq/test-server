...
exports.updatePassword = async (req) => {
  ...
  if ((md5(`++${curr_pass}--`) === valueData[0]?.password)) {
    const previous_password = await knex("APSISIPDC.cr_password_history")
      .where("user_id", id)
      .select(
        "id",
        "password"
    ...
    for (let i = 0; i < previous_password.length; i++) {
      if (new_pass === previous_password[i].password) {
        matchPrevPass = 1;
        break;
      }
    }
    if (matchPrevPass == 0) {
      ...
      return sendApiResult(false, "It's old password! Please enter new password.")
    }
  } else {
    return sendApiResult(false, "Current Password Is Not Same.", valueData[0]?.id);
  }
