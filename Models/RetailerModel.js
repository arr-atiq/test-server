      ...
      await knex
        .transaction(async (trx) => {
          ...
          if (Object.keys(rows).length !== 0) {
            ...
            const insertRetailerList = await knex("APSISIPDC.cr_retailer_temp").insert(retailerList);
            if (insertRetailerList == true) {
              ...
              const uploadLog = await knex("APSISIPDC.cr_retailer_upload_log").insert(insertLog);
              if (uploadLog == true) {
...
Retailer.checkRetailerEligibility = function (req) {
  return new Promise(async (resolve, reject) => {
    try {
      const distributorSql = await knex.from("APSISIPDC.cr_distributor")
      ...
      const retailerTypeSql = await knex.from("APSISIPDC.cr_retailer_type")
        .select("id", "name")
        .where("status", "Active");
      ...
      const retailerTypeEntitySql = await knex.from("APSISIPDC.cr_retailer_type_entity")
        .select("id", "name")
        .where("status", "Active");
      ...
      const bulkRetailerInfoList = await knex.from("APSISIPDC.cr_retailer_temp")
        .select()
      ...
      if (Object.keys(bulkRetailerInfoList).length != 0) {
        for (const [key, value] of Object.entries(bulkRetailerInfoList)) {
          ...
          if (validNID == true && validMonthlySalesData == true) {
            ...
            const checkMasterRetailer = await knex.from('APSISIPDC.cr_retailer')
              .select('id')
            ...
            if (Object.keys(checkMasterRetailer).length == 0) {
              ...
              var r_number_rmn = await knex('APSISIPDC.cr_retailer_manu_scheme_mapping')
              ...
              const mappingRetailerInsertLog = await knex('APSISIPDC.cr_retailer_manu_scheme_mapping').insert(retailerManuDistMappingInsert);
            ...
            const checkRetailerManuMapping = await knex.from('APSISIPDC.cr_retailer_manu_scheme_mapping')
              .select('id')
              .where('retailer_code', value.retailer_code)
              .where('manufacturer_id', value.manufacturer);
    ...
    try {
      ...
      if (Object.keys(retailerList).length !== 0) {
        for (const [key, value] of Object.entries(retailerList)) {
          ...
          await knex('APSISIPDC.cr_retailer_credit_limit_history').insert(retailerLimitHistory);
          ...
          await knex('APSISIPDC.cr_retailer_sales_volume').insert(retailerSalesVolume);
        };
      } else {
        var msg = 'No data Found';
        reject(sendApiResult(false, msg));
